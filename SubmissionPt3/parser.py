...
#this function just grabs the line of the last token
def getLine(i):
    global tokenList
    #print(tokenList[i].line)
    return tokenList[i].line
...
def checkFunctionDecl():
    ...
    if stmtBlock.finished:
        ...
        functionDecl.line = getLine(originalIndex)
...
def checkStmtBlock():
    ...
    stmtBlockClass.line = originalIndex
    return stmtBlockClass
...
def checkStmt():
    ...
    if checkOptionalSemiColon():
        stmt.finished = True
        stmt.line = getLine(originalIndex)
        stmt.expr = None
        return stmt
    ...
    if tokenList[index].text == "Print":
        ...
        if checkLParen():
            ...
            if checkRParen() and checkSemiColon() and len(printStmt.exprs) > 0:
                ...
                printStmt.line = getLine(originalIndex)
            ...
            if expr.finished:
                ...
                if checkRParen():
                    ...
                    if thenStmt.finished:
                        ...
                        ifStmt.line = getLine(originalIndex)
            else:
                printError(index)
    if ifStmt.finished:
        originalIndex = index
...
def checkReturnStmt():
    ...
    #print("DEAUX")
        if checkSemiColon():
            ...
            returnStmt.line = getLine(originalIndex)
        else:
            index = originalIndex
...
def checkForStmt():
    ...
    if tokenList[index].text == "for":
        ...
        if stmt.finished:
            ...
            forStmt.line = getLine(originalIndex)
...
def checkCall():
    ...
    #print("WHOOPS")
    call.finished = True
    call.line =  getLine(originalIndex)
    #call.printMyStuff(0)
    return call
...
def checkReadThing():
    ...
    if tokenList[index].text == "ReadInteger":
        ...
        if checkLParen() and checkRParen():
            ...
            checkReadClass.line = getLine(originalIndex)
        ...
        if checkLParen() and checkRParen():
            ...
            checkLineClass.line = getLine(originalIndex)
